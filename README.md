...
| `create()`           | `filePath?`: `string`<br>`options`: `{ overwrite?: boolean }`                      | Creates a new log file at the specified or default path. `overwrite: false` prevents overwriting an existing file.          | `Promise<void>`                         |
| `write()`            | `log`: `LogMessage`<br>`filePath?`: `string`<br>`options?`: `{ append?: boolean }` | Writes a single, formatted log message directly to the log file, **default append:** `true`. Does **not** use the buffer.   | `Promise<void>`                         |
| `read()`             | `filePath?`: `string`                                                              | Reads the content of the log file. The return type depends on the file extension (`string` for text, `object[]` for JSONL). | `Promise<string \| LogMessage[] \| []>` |
| `remove()`           | `filePath?`: `string`                                                              | Deletes the log file.                                                                                                       | `Promise<void>`                         |
...
| Method    | Parameters                                                                   | Description                                                                                                                                                            | Return Type     |
| --------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- |
| `push()`  | `log`: `LogMessage`                                                          | Adds a new message to the buffer's `data` array.                                                                                                                       | `void`          |
| `write()` | `filePath?`: `string`<br>`options?`: `{ flush?: boolean, append?: boolean }` | Writes the entire buffer content to the log file. **Default append:** `true`. The buffer is flushed after writing. Set `flush: false` to retain the buffer's contents. | `Promise<void>` |
